...
export class EmailAlertHandler {
    ...
    if (this.config.enabled && this.config.smtpHost) {
      try {
        this.transporter = nodemailer.createTransport({
          ...
          auth: this.config.smtpUser && this.config.smtpPass
            ? {
        ...
        this.transporter.verify().then((success) => {
          ...
          console.warn("[Alert] Email alerts may not work correctly - please check SMTP configuration");
          this.verified = false;
        });
      } catch (error) {
  ...
  async sendAlert(alert: AlertContext): Promise<boolean> {
    ...
    if (!this.transporter) {
      console.warn("[Alert] SMTP transporter not configured, cannot send email");
    ...
    for (let attempt = 0; attempt < this.maxRetries; attempt++) {
      try {
        ...
        console.error(`[Alert] Email send attempt ${attempt + 1}/${this.maxRetries} failed:`, error);
  ...
  async sendAlert(alert: AlertContext): Promise<boolean> {
    ...
    for (let attempt = 0; attempt < this.maxRetries; attempt++) {
      try {
        ...
        if (response.ok) {
          console.log(`[Alert] Webhook sent successfully to ${this.config.url}`);
          return true;
        }
        
        console.warn(`[Alert] Webhook failed with status ${response.status}, attempt ${attempt + 1}/${this.maxRetries}`);
        
        if (attempt < this.maxRetries - 1) {
          await new Promise(resolve => setTimeout(resolve, Math.pow(2, attempt) * 1000));
        }
      } catch (error) {
        console.error(`[Alert] Webhook error on attempt ${attempt + 1}:`, error);
